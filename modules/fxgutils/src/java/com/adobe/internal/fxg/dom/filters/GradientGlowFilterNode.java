...
public class GradientGlowFilterNode extends AbstractFilterNode
{
    ...
    public void addChild(FXGNode child)
    {
        if (child instanceof GradientEntryNode)
        {
            ...
            else if (entries.size() >= GRADIENT_ENTRIES_MAX_INCLUSIVE)
            {
                ...
                FXGLog.getLogger().log(FXGLogger.WARN, "InvalidGradientGlowFilterNumElements", null, getDocumentName(), startLine, startColumn);
                return;
            }
    ...
    /** 
     * Set gradient glow filter properties. Delegates to the parent class
