...
public class AbstractProducer extends MessageAgent
{
    ...
    /**
     *  If true the Producer will automatically connect to its destination the
     *  first time <code>send()</code> is invoked.
     *  If false then <code>connect()</code> must be invoked explicitly to 
     *  establish a connection to the destination.
     *  By default this property is true but applications that need to operate
     *  in an offline fashion may set this to false to prevent <code>send()</code>
     ...
     *  supress pending acks and faults.
	 */
