...
public class SchemaMarshaller //implements IXMLTypeMarshaller
{
    ...
    /*
        var elements:XMLList = definition.elements();
        for each (var child:XML in elements)
        {
            // <enumeration>
            if (child.name() == constants.enumerationTypeQName)
            {
                validated = validateEnumeration(value, definition);
                if (validated)
                {
                    isType = true;
                }
                ...
                {
                    throw new Error(tokenType.localName + " does not satisfy enumeration values");
                }
            }
        ...
        //check for patterns
        if (definition.child(schemaConstants.patternQName).length() > 0)
        {
            childXML = restrictionXML.child(schemaConstants.patternQName);
            var pattern:String = childXML.@value;
            ...
            {
                throw new Error(tokenType.localName + " does not satisfy the pattern");
            }
        } 
         
        //check for mininclusive and maxinclusive
        if (restrictionXML.child(schemaConstants.maxInclusiveQName).length() > 0 ||
           restrictionXML.child(schemaConstants.minInclusiveQName).length() > 0 )
         {
            var max:Number;
            var min:Number;
            ...
            { 
                ...
                {
                    if (childXML.name() == schemaConstants.minInclusiveQName)
                    {
                        min = parseInt(childXML.@value);
                        
                    }
                    if (childXML.name() == schemaConstants.maxInclusiveQName)
                    {
                        max = parseInt(childXML.@value);
                    }    
                }
                num = Number(value);
                if (num >= min && num <= max)
                ...
                {
                    throw new Error(num + " outside the range.");
                }
            }                      
            else if (restrictionXML.child(schemaConstants.maxInclusiveQName).length() > 0)
            {
                childXML = restrictionXML.child(schemaConstants.maxInclusiveQName);
                max = parseInt(childXML.@value);
                num = Number(value);
                ...
                {
                    throw new Error(num + " greater than maximum: " +  max);
                }
            }
            else if (restrictionXML.child(schemaConstants.minInclusiveQName).length() > 0 )
            {
                childXML = restrictionXML.child(schemaConstants.minInclusiveQName);
                min = parseInt(childXML.@value);
                num = Number(value);
                ...
                {
                    throw new Error(num + " less than minimum: " + min);
                }
            }
             
         }
         //check for max and min length
         if (restrictionXML.child(schemaConstants.maxLengthQName).length() > 0 ||
                                  restrictionXML.child(schemaConstants.minLengthQName).length() > 0)
         {
            var max1:Number;
            var min1:Number;
                                  ...
                                  restrictionXML.child(schemaConstants.minLengthQName).length() > 0)
            {
                for each (childXML in childXMLList)
                {
                    if (childXML.name() == schemaConstants.minLengthQName)
                    {
                        min1 = parseInt(childXML.@value);
                    }
                    if (childXML.name() == schemaConstants.maxLengthQName)
                    {
                        max1 = parseInt(childXML.@value);
                    }    
                }
                str = String(value);
                if (str.length >= min1 && str.length <= max1)
                {
                     isType = true;
                }
                else
                {
                    throw new Error(str + " outside the range.");
                }
            }
            else if (restrictionXML.child(schemaConstants.maxLengthQName).length() > 0)
            {
                childXML = restrictionXML.child(schemaConstants.maxLengthQName);
                max1 = parseInt(childXML.@value);
                str = String(value);
                if (str.length <= max1)
                {
                ...
                {
                    throw new Error(str + " greater than maximum: " +  max1);
                }
            }
            else if (restrictionXML.child(schemaConstants.minLengthQName).length() > 0)
            {
                childXML = restrictionXML.child(schemaConstants.minLengthQName);
                min1 = parseInt(childXML.@value);
                str = String(value);
                if (str.length >= min1)
                {
                ...
                {
                    throw new Error(str + " less than minimum: " + min1);
                }
            }
         }
         
         //check for length
         if (restrictionXML.child(schemaConstants.lengthQName).length() > 0)
         {
            childXML = restrictionXML.child(schemaConstants.lengthQName); 
            num = parseInt(childXML.@value);
            str = String(value);
            if (str.length == num)
            {
                isType = true;
            ...
            {
                throw new Error(str + " lengths does not match");
            }
         }
         if (isType == true)
         {
            var baseName:String = restrictionXML.@base;
            var baseQName:QName = getQNameForPrefixedName(baseName, restrictionXML);
            childBody = encodeBuiltInType(tokenType, baseQName, value);
            var retChildBody:XMLList = new XMLList(childBody);
            return retChildBody;
         }
         ...
         {
             throw new Error("Cannot be added to body tag");
         }
         
         return null;
         
         //[FIXME] validate sequence, all etc
 
 
   */
   
   /*     
    // 1. get the enumeration type values and validate against the arguments
    private function validateEnumeration(value:*, restrictionXML:XML):Boolean
    {
        var schemaConstants:SchemaConstants = schemaManager.schemaConstants;

        var list:Array = [];
        var i:uint;
        var enumXMLList:XMLList = restrictionXML.child(schemaConstants.enumerationTypeQName);
        var enumXML:XML;

        //get the values in an array 
        for each (enumXML in enumXMLList)
        {
            var value1:String = enumXML.@value;
            list.push(value1);
        }
