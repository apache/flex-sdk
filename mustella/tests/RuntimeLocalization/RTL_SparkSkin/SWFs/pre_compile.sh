...
then
    ...
    $SDK_DIR/bin/compc -library-path=$LIBRARY_PATH -source-path=bundles/framework/de_DE -locale de_DE -include-resource-bundles collections components containers controls core effects formatters layout logging SharedResources skins states styles validators textLayout sparkEffects -o bundles/framework/deResources.swc
    $SDK_DIR/bin/compc -source-path=bundles/framework/en_US -locale en_US -include-resource-bundles collections components containers controls core effects formatters layout logging SharedResources skins states styles validators textLayout -o bundles/framework/enResources.swc
    $SDK_DIR/bin/compc -library-path=$LIBRARY_PATH -source-path=bundles/framework/fr_FR -locale fr_FR -include-resource-bundles collections components containers controls core effects formatters layout logging SharedResources skins states styles validators textLayout sparkEffects -o bundles/framework/frResources.swc
    $SDK_DIR/bin/compc -library-path=$LIBRARY_PATH -source-path=bundles/framework/ja_JP -locale ja_JP -include-resource-bundles collections components containers controls core effects formatters layout logging SharedResources skins states styles validators textLayout sparkEffects -o bundles/framework/jaResources.swc
    $SDK_DIR/bin/compc -library-path=$LIBRARY_PATH -source-path=bundles/framework/qa_QA -locale qa_QA -include-resource-bundles collections components containers controls core effects formatters layout logging SharedResources skins states styles validators textLayout sparkEffects -o bundles/framework/qaResources.swc
